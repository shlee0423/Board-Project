...
public class UserService {
    ...
    private String PORT_ONE_IMP_KEY;

    @Value("${PORT_ONE_IMP_SECRET}")
    private String PORT_ONE_IMP_SECRET;

    private final String PORT_ONE_ACCESS_TOKEN_URL = "https://api.iamport.kr/users/getToken";
    private final String PORT_ONE_USER_CERT_INFO_URL = "https://api.iamport.kr/certifications/%s";
    ...
    // 유저 생성 (회원가입)
    public boolean createUser(String impUid, UserDTO user){
        if(impUid.isBlank()){
            return false;
        }
    ...
    public boolean checkUsername(String id){
        int count = userMapper.selectUsername(id);
        return count > 0;
    }
}
